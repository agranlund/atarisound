...
{
  unsigned char *lastrows=malloc(m->patnum);
  if (!lastrows)
    return;
...
{
  unsigned short* rellist=malloc(m->sampnum * sizeof(unsigned short));
  if (!rellist)
    return 0;
...
{
  m->instnum=n;
  m->instruments=malloc(m->instnum*sizeof(gmdinstrument));
  if (!m->instruments)
    return 0;
...
{
  m->tracknum=n;
  m->tracks=malloc(m->tracknum*sizeof(gmdtrack));
  if (!m->tracks)
    return 0;
...
{
  m->patnum=n;
  m->patterns=malloc(m->patnum*sizeof(gmdpattern));
  if (!m->patterns)
    return 0;
...
{
  m->sampnum=n;
  m->samples=malloc(m->sampnum*sizeof(sampleinfo));
  if (!m->samples)
    return 0;
...
{
  m->envnum=n;
  m->envelopes=malloc(m->envnum*sizeof(gmdenvelope));
  if (!m->envelopes)
    return 0;
...
{
  m->modsampnum=n;
  m->modsamples=malloc(m->modsampnum*sizeof(gmdsample));
  if (!m->modsamples)
    return 0;
