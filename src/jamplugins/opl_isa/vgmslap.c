...
#define hookTimerA      jamHookTimerA
#define unhookTimerA    jamUnhookTimerA
...
static inline int isaInit() {
    #define C__ISA  0x5F495341 /* '_ISA' */
    return (Getcookie(C__ISA, (long*)&isa) == C_FOUND) ? 1 : 0;
}
...
void vgm_delay(int ms) {
    // used in opl detection and reset, accuracy is not super important
    // timerc may or may not be running at this point so provide simple fallback.
    clock_t start  = clock();
    clock_t cycles = ms / 5;
    cycles = cycles ? cycles : 1;
    long maxloops = ms * 100;
	do {
        maxloops -= 1;
        isa->inp(0x80);
    } while ((clock() - start < cycles) && (maxloops > 0));
}
...
uint8_t vgmslap_init()
{
    dbgprintf("vgmslap_init\r\n");
    if (!isaInit()) {
        return 0;
    }
...
void initTimer(uint16_t frequency)
{
    ...
    uint32_t hz_real = hookTimerA(timerHandler, hz);
